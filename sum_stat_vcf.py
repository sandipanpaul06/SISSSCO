...
parser.add_argument('name', type=str, help= 'Parsed VCF file name')
...
path_1 = os.getcwd()
path = path_1 + "/Parsed_VCF/" + str(val)
...
for i in range(1):
    ...
    pi_ = pi_stat(matrix = np.load(path_1 + "/Parsed_VCF/" + str(val)), num_strands= 198, window_length=10, num_stride=3)
    if pi_ != None:
        stats_9.append(pi_)
        #clear_output(wait=True)
        #print(i)
...
for i in range(1):
    #if len(pi_list_sweep_train)<1000:
    h1_, h12_, h2h1_,p1_, p2_, p3_, p4_, p5_ = all_stat(matrix = np.load(path_1 + "/Parsed_VCF/" + str(val)), num_strands= 198, window_length=10, num_stride=3)
    if pi_ != None:
        stats_9.append(h1_)
        stats_9.append(h12_)
